...
export const AIBackground: React.FC<AIBackgroundProps> = ({ animate = true }) => {
  ...
  return (
    <div className="fixed inset-0 z-[0] overflow-hidden pointer-events-none bg-transparent">
      <div className={cn("absolute inset-0 ai-gradient-bg bg-noise", theme === 'dark' ? 'opacity-40' : 'opacity-30')} />
      <canvas
        ref={canvasRef}
        className={cn("absolute inset-0 w-full h-full transition-opacity duration-1000", theme === 'dark' ? 'opacity-100' : 'opacity-90')}
      />
    </div>
  );
};
