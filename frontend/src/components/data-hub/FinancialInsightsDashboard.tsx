...
// Dynamic Insights Component
export function DynamicInsights({ aiResponse }: { aiResponse: any }) {
        ...
        <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
            ...
            {aiResponse.narrative && (
                <Card className="border-blue-500/20 bg-blue-500/5">
                    <CardHeader className="pb-2"><CardTitle className="text-sm font-bold text-blue-500">AI Analysis</CardTitle></CardHeader>
                    <CardContent className="text-sm">{aiResponse.narrative}</CardContent>
                </Card>
            )}
            ...
            {aiResponse.has_risk && (
                <div className="bg-red-500/10 border-l-4 border-red-500 p-4 rounded-r-md">
                    <p className="font-bold text-red-500 text-sm">Risk Alert</p>
                    <p className="text-xs text-red-400">{aiResponse.risk_details}</p>
                </div>
            )}

            {/* 3. Generated SQL */}
            {aiResponse.sql_query && (
                <div className="bg-slate-950 text-slate-50 p-4 rounded-md font-mono text-xs overflow-x-auto border border-white/5">
                    <span className="text-green-400 font-bold">{">"} Generated Query:</span> {aiResponse.sql_query}
                </div>
            )}
        </div>
    );
...
export default function FinancialInsightsDashboard({ view = 'executive' }: { view?: 'executive' | 'finance' | 'department' }) {
    ...
    const fetchTruth = async () => {
        setLoading(true);
        try {
            // Targeted Metrics Endpoint
            const endpoint = "/api/process_transaction/metrics";
    ...
    useEffect(() => {
        ...
        const q = query(
            collection(db, 'sgg_transactions'),
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            ...
            const filteredData = allData.filter(_txn => {
                // Adapting filtering since sgg_transactions structure might differ slightly or be simpler
                // Assuming sgg_transactions has 'company_id' or we infer it.
                // For now, relax filtering or assume context matches.
                // If sgg_transactions only has {transaction_id, date, amount}, we might not filter by company yet.
                // But let's verify if we need to keep filtering.
                // Transformation script doesn't add company_id.
                // So let's show all for now or check if we should add it in transformation.
                // User didn't ask to update transformation for company_id.
                // So I will just return all data for now or allow filtering if fields exist.
                return true;
            });
    ...
    useEffect(() => {
        const fetchForecast = async () => {
            ...
            try {
                ...
                const itemCode = forecastItem || 'MSFT';
                ...
                const res = await fetch('/api/process_transaction/metrics', {
                    ...
                    body: JSON.stringify({
                        action: 'forecast',
                        company_id: selectedCompany || 'SGG-001',
                        itemCode: itemCode,
                        forecast_horizon: 12
                    })
                });
                ...
                if (result.success && result.forecast) {
                    ...
                    const adjustedData = result.forecast.map((d: any) => ({
                        date: d.ds,
                        forecast: d.yhat,
                        upper: d.yhat_upper,
                        lower: d.yhat_lower,
                        actual: undefined // Actuals might be separate
                    }));
                    setChartData(adjustedData);
                }
            } catch (error) {
                console.error("AI Fetch Failed", error);
                setChartData([]); // Clear data on error instead of mock
            } finally {
                setLoading(false);
            }
        };
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
                <div className="flex flex-col gap-1">
                    ...
                    {/* For demo purposes, we pass a mock response or state if available. 
                    In real app, this would come from the AI Assistant context or specific fetch. */}
                    {/* <DynamicInsights aiResponse={{ narrative: "Quarterly revenue exceeds forecast by 12%.", has_risk: false, sql_query: "SELECT * FROM revenue WHERE q='Q3'" }} /> */}
